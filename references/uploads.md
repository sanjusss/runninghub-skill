...
```bash
python3 scripts/rh.py upload photo.png        # → data.fileName + download_url
```
...
```bash
python3 scripts/rh.py upload-lora my-style.safetensors --name 我的风格
...
```bash
python3 scripts/rh.py workflow <id> --node "15:lora_name=api-lora-cn/<md5>.safetensors"
```

注意：
- 该通道上传的 LoRA **只有 RHLoraLoader 认识**，原生 LoraLoader 读不到
- md5 是缓存键：同文件重复"上传"直接命中缓存（服务端可能不返回 url，此时直接用 fileName）
...
```bash
python3 scripts/rh.py resources --type CHECKPOINT --kw flux --size 20
